...
module.exports = {
	...
	getSimilarGamers: function(req, res){
		var id = req.user._id;
		...
		Promise.all(promiseArr).then(function(data){
			...
			Service.getCompareUsers(idArr).then(function(users){
				for (var i = 0; i < users.length; i++) {
					var userGames = users[i].games;
					userGames = userGames.map(function(game) {
					  return game.title;
					});
				}
				Service.getSimilarGamers(req.user, users, function(nearestNeighbor, probability){
					var matchedGamer = {};
					matchedGamer.user = nearestNeighbor;
					matchedGamer.percentage = probability;
