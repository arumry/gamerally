	...
	getSimilarGamer: function(req, res){
		...
		Promise.all(promiseArr).then(function(data){
			...
			Service.getCompareUsers(idArr).then(function(users){
				Service.getSimilarGamer(user, users, function(gamers, probability){
					var matchedGamers = {};
					matchedGamers.users = gamers;
					matchedGamers.topPercentage = probability * 100;
					res.status(200).send(matchedGamers);
				});
			});
		});
	},
	declineGamer: function(req, res){
		var id = req.user._id;
		var id2 = new ObjectId(req.params.id);
		Service.addToCounterCheck(id, id2).then(function(result){
			res.status(200).end();
		});
	}
};