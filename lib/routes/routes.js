var Ctrl = require('../controllers/userCtrl');
var middleware = require('../middleware/middleware');

module.exports = function(app, passport){
	//Steam auth and callback url endpoints
    app.get('/auth/steam', passport.authenticate('steam', { failureRedirect: '/#/login' }), Ctrl.noop);
    app.get('/auth/steam/return', passport.authenticate('steam', { failureRedirect: '/#/login', successRedirect: '/#/profile' }), Ctrl.steamLogin); 
    app.get('/auth/steam/logout', middleware.checkAuth, Ctrl.steamLogout);
    
    //User endpoints
    app.get('/user', middleware.checkAuth, Ctrl.getUser);

    //Game endpoints
    app.get('/game', middleware.checkAuth, Ctrl.searchGames);
    app.get('/games', middleware.checkAuth, Ctrl.getGames);
    app.post('/game', middleware.checkAuth, Ctrl.addGame);
    app.put('/game/:id', middleware.checkAuth, Ctrl.editGame);
    app.delete('/game/:id', middleware.checkAuth, Ctrl.delGame);

    //Friend endpoints
    app.get('/friends/pending', middleware.checkAuth, Ctrl.getPendingFriends);
    app.get('/friends/accepted', middleware.checkAuth, Ctrl.getAcceptedFriends);
    app.get('/friends/requested', middleware.checkAuth, Ctrl.getRequestedFriends);
    app.delete('/friend/:id', middleware.checkAuth, Ctrl.deleteFriend);
    app.post('/friend/request/:id', middleware.checkAuth, Ctrl.requestFriend);

    //Message endpoints
      //will need to check that user can send to user (check if the user exists in 
        //their accepted friends list, swal on frontend if there is a problem)

};